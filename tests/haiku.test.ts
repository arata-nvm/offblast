...
describe("moraCount", () => {
  ...
  it("記号・未知語は0", () => {
    ...
    expect(moraCount(tok({ surface: "𠮷", pron: "*", isUnknown: true }))).toBe(0);
  });
});
...
describe("findHaikus（純ロジック・ゴールデン）", () => {
  const constitution: Token[] = [
    ...
    tok({ surface: "の", pron: "ノ", pos: "助詞", posDetail1: "連体化", basicForm: "の" }),
    tok({ surface: "自由", pron: "ジユー", posDetail1: "形容動詞語幹" }),
    tok({ surface: "は", pron: "ワ", pos: "助詞", posDetail1: "係助詞", basicForm: "は" }),
    tok({ surface: "これ", pron: "コレ", posDetail1: "代名詞", basicForm: "これ" }),
    tok({ surface: "を", pron: "ヲ", pos: "助詞", posDetail1: "格助詞", basicForm: "を" }),
    tok({ surface: "保障", pron: "ホショー", posDetail1: "サ変接続" }),
    tok({ surface: "する", pron: "スル", pos: "動詞", posDetail1: "自立", basicForm: "する" }),
  ];
  ...
  it("助詞始まりは句の先頭にできない", () => {
    const t = [tok({ surface: "の", pron: "ノ", pos: "助詞", posDetail1: "連体化" }), ...constitution];
...
describe("clean と XML パイプライン", () => {
  it("割注（…）と非句点文を除去する", () => {
    expect(cleanBody(["学問の自由は（例外を除く）これを保障する。", "断片"])).toBe(
      "学問の自由はこれを保障する。"
    );
  });
  ...
  it("法令XMLの Sentence から575を検出できる", async () => {
    ...
    expect(await detectFromSentences(sentences)).toContain("学問の 自由は、これを 保障する");
  });
});
